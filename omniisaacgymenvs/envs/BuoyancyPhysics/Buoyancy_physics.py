...
class BuoyantObject:
    def __init__(self, num_envs, water_density, gravity, metacentric_width, metacentric_length ):
            ...
            self.archimedes_force_global = torch.zeros((self._num_envs,  3), dtype=torch.float32)
            self.archimedes_torque_global = torch.zeros((self._num_envs,  3), dtype=torch.float32)
            self.archimedes_force_local = torch.zeros((self._num_envs, 3), dtype=torch.float32)
            self.archimedes_torque_local = torch.zeros((self._num_envs, 3), dtype=torch.float32)
    ...
    def compute_archimedes_metacentric_global(self, submerged_volume, rpy):
        ...
        print("self.archimedes_force global: ", self.archimedes_force_global)
        print("self.archimedes_torque global: ", self.archimedes_torque_global)
    ...
    def compute_archimedes_metacentric_local(self, submerged_volume, rpy, quaternions):
        ...
        #get rotation matrix from quaternions in world frame
        #size is (3*num_envs, 3)
